...
Citizen.CreateThread(function()
	while true do
			...
			if CurrentLocation ~= nil then
					...
					AddTextComponentString('Press ~b~~h~E~h~~w~ klaxonner pour appler la pute')
					DisplayHelpTextFromStringLabel(0, 0, 1, -1)
					if IsControlJustPressed(0,51) then
						TaskEnterVehicle(SpawnedPed, GetVehiclePedIsIn(playerPed, false), -1, 0, 1.0, 1, 0)
						Citizen.Wait(5000)
						TriggerEvent("chatMessage", "Veronica", {0 , 255, 0 }, "Hé mon loulou, que voudrais-tu ? Une fellation ou quelque chose de mieux?")
						Citizen.Wait(1000)
						TriggerEvent("chatMessage", "INFO", {0 , 255, 0 }, "Taper /bj pour une bonne pipe des familles, /sex ou /getout pour qu'elle dégage")
					end
			end
	end
end)
